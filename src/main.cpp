...
struct Point {
  ...
  bool check(int xx, int yy) const { return x == xx && y == yy; }
  bool operator==(const Point &pt) const { return check(pt.x, pt.y); }
};
...
void drawBattery() {
  ...
  int br = (int)((float)M5.Power.getBatteryLevel() * 0.56f);
  ...
  while (1) {
    ...
    if (touchCount > 0) {
      for (int i = 0; i < touchCount; i++) {
        auto FingerItem = M5.Touch.getDetail(i);
